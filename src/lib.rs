...
mod NBT {
    ...
    pub enum Tag {
        ...
        TagFloat(f32),
        TagDouble(f64),
        TagByteArray(i32, Vec<i8>),
        TagString(String),
        TagList(i8, i32, Vec<Tag>),
    ...
    impl Tag {
        ...
        fn pretty_print(&self, indent: usize, name: Option<&String>) {
            ...
            match self {
                ...
                &Tag::TagList(ty, len, ref l) => { println!("{1:0$}{2}{3} : {4} entries of type {5}\n{1:0$}{{",
                                                           indent,"", self.get_name(), name_s, l.len(), ty);
                                                  for item in l.iter() {item.pretty_print(indent+4, None); }
                                                  println!("{1:0$}}}", indent, "");
                                                 }
                &Tag::TagString(ref s) => { println!("{1:0$}{2}{3} : {4}", indent, "", self.get_name(), name_s, s) }
                &Tag::TagByteArray(len, _) => { println!("{1:0$}{2}{3} : Length of {4}", indent, "", self.get_name(), name_s, len); }
...
fn test_level_dat() {
    ...
    let tag = NBT::NamedTag::parse(&mut decoder);
    tag.pretty_print();
}
...
fn test_tag_byte() {
    ...
    if let NBT::Tag::TagByte(b) = tag.tag {
        assert_eq!(b, 69);
    } else {
...
fn test_tag_byte_array() {
    ...
    let tag = NBT::NamedTag::parse(&mut cur);
    assert_eq!(tag.name, "hello");
    if let NBT::Tag::TagByteArray(l, v) = tag.tag {
        assert_eq!(l, 3);
        assert_eq!(v, vec!(69, -6, 123));
    } else {
        panic!("Unexpected tag type!");
    }
}
...
fn test_tag_string() {
    ...
    let tag = NBT::NamedTag::parse(&mut cur);
    assert_eq!(tag.name, "hello");
    if let NBT::Tag::TagString(s) = tag.tag {
        assert_eq!(s, "cat");
    } else {
        panic!("Unexpected tag type!");
    }
...
fn test_tag_list() {
    ...
    if let NBT::Tag::TagList(t, l, v) = tag.tag {
        assert_eq!(t, 1);
        assert_eq!(l, 3);
        assert_eq!(v[0], NBT::Tag::TagByte(1));
        assert_eq!(v[1], NBT::Tag::TagByte(2));
        assert_eq!(v[2], NBT::Tag::TagByte(3));
    } else {
